...
import { LayoutDashboard, PlusCircle, Settings, LogOut, UploadCloud, CheckCircle, Store, Sparkles, Plus, Trash2, Megaphone, Edit3, Pin, ChevronDown, ChevronUp, Users, MessageCircle, MapPin, CalendarCheck, Share, Clock, Search, ChevronRight, X } from 'lucide-react';
...
// Create custom icon function for therapists
let L: any = null;
...
const createTherapistIcon = (imageUrl: string) => {
    if (!L) return null;
    return L.divIcon({
        html: `<div style="width: 48px; height: 48px; border-radius: 50%; overflow: hidden; border: 3px solid #0A84FF; box-shadow: 0 8px 16px rgba(0,0,0,0.4); background: white;"><img src="${imageUrl || 'https://ui-avatars.com/api/?name=T&background=random'}" style="width: 100%; height: 100%; object-fit: cover;" /></div>`,
        className: 'custom-therapist-icon',
        iconSize: [48, 48],
        iconAnchor: [24, 24],
        popupAnchor: [0, -24]
    });
...
export default function AdminDashboard() {
    ...
    const handleTherapistStatus = async (id: string, is_active: boolean) => {
        try {
            const { error } = await supabase.from('therapists').update({ is_active }).eq('id', id);
            if (error) {
            ...
            <aside className="hidden md:flex flex-col w-64 bg-white/5 border border-white/20 border-r border-white/20/50 shadow-soft z-20">
                ...
                <nav className="flex-1 px-4 py-6 space-y-2">
                    ...
                    <button 
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-colors ${activeTab === 'book' ? 'bg-[#0A84FF]/20 text-[#0A84FF]' : 'text-white/90-muted hover:bg-surface/50 hover:text-white'}`}
                    >
                                                ...
                                                <div className="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0 hide-scrollbar">
                                                    {['All', 'Downtown Dubai', 'Dubai Marina', 'Jumeirah'].map(loc => (
                                                        <button 
                                                            ...
                                                            className={`px-4 py-2 rounded-full text-xs font-bold transition-all whitespace-nowrap ${locationFilter === loc ? 'bg-white/20 text-white shadow-[0_0_15px_rgba(255,255,255,0.1)]' : 'bg-transparent border border-white/10 text-white/50 hover:border-white/30 hover:text-white/80'}`}
                                                        >
                                                            {loc}
                                                        </button>
                                                    ))}
                                                            ...
                                                            {searchedBookings.map((booking) => {
                                                                const { companyName } = (() => {
                                                                    ...
                                                                    if (booking.treatments && booking.treatments.length > 0) {
                                                                        const treatmentDetails = treatments.find((t: any) => t.id === booking.treatments[0].treatmentId);
                                                                        if (treatmentDetails && treatmentDetails.therapist_id) {
                                                                            const partner = allTherapists.find(t => t.id === treatmentDetails.therapist_id);
                                                                            if (partner) companyName = partner.brand || partner.name;
                                                                        }
                                                                    }
                                                                    return { companyName };
                                                                })();
                                                    ...
                                                    >
                                                        ...
                                                        <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-8">
                                                            ...
                                                            {(() => {
                                                                ...
                                                                if (selectedBooking.treatments && selectedBooking.treatments.length > 0) {
                                                                    const treatmentDetails = treatments.find((t: any) => t.id === selectedBooking.treatments[0].treatmentId);
                                                                    if (treatmentDetails && treatmentDetails.therapist_id) {
                                                                        const partner = allTherapists.find(t => t.id === treatmentDetails.therapist_id);
                                                                        if (partner) {
                                                                    ...
                                                                    <>
                                                                        ...
                                                                        <div className="flex items-center justify-between">
                                                                            ...
                                                                            <a 
                                                                                ...
                                                                                target="_blank"
                                                                                className="w-10 h-10 rounded-xl bg-[#0A84FF]/10 text-[#0A84FF] flex items-center justify-center hover:bg-[#0A84FF]/20 transition-colors"
                                                                            >
                                                                        ...
                                                                        <div className="flex flex-col gap-3 mt-4 pt-6 border-t border-white/[0.04]">
                                                                            <button 
                                                                                onClick={(e) => {
                                                                                    e.preventDefault();
                                                                                    const treatmentsStr = selectedBooking.treatments.map((t: any) => `${t.title} (${t.duration} Minutes for ${t.guests} Persons)`).join('\n');
                                                                                    const msg = `*NEW SPA BOOKING ASSIGNMENT*\n\n*CLIENT DETAILS:*\n- Name: ${selectedBooking.customer_name}\n- Date: ${monthWord} ${dayNum}, ${yearNum}\n- Time: ${selectedBooking.time}\n- Location: ${selectedBooking.location_area} - ${selectedBooking.address}\n- Room Number: ${selectedBooking.room_number || '-'}\n\n*TREATMENTS:*\n${treatmentsStr}\n\n*TOTAL PRICE:* AED ${selectedBooking.total_price.toLocaleString('en-US')}\n\nPlease confirm if you can take this booking!`;
                                                                                    const phone = companyPhone.replace(/[^0-9]/g, '');
                                                                                    window.open(`https://wa.me/${phone}?text=${encodeURIComponent(msg)}`, '_blank');
                                                                                }}
                                                                                type="button"
                                                                                className="w-full bg-[#25D366] text-white rounded-xl py-3.5 font-bold text-xs tracking-wider hover:bg-[#20bd5a] transition-all flex items-center justify-center gap-2 shadow-lg shadow-[#25D366]/20"
                                                                            >
                                                                                <Share className="w-4 h-4" /> Dispatch to Partner
                                                                            </button>
                                                                            <div className="w-full">
                                                                                <InvoiceGenerator booking={selectedBooking} companyName={companyName} />
                                                                            </div>
                                                                        </div>
                                                                    </>
                                                                );
                            ...
                            <div className="p-8 overflow-y-auto flex-1 space-y-6">
                                ...
                                <div className="pt-6 flex flex-col sm:flex-row items-center justify-between border-t border-white/5 gap-4">
                                    ...
                                    <div className="flex items-center justify-between w-full sm:w-auto bg-white/5 sm:bg-transparent px-4 py-3 sm:p-0 rounded-xl sm:rounded-none">
                                        ...
                                        <button 
                                            onClick={() => {
                                                handleTherapistStatus(detailedTherapist.id, !detailedTherapist.is_active);
                                                setDetailedTherapist({ ...detailedTherapist, is_active: !detailedTherapist.is_active });
                                            }}
                                            className={`relative inline-flex h-8 w-14 items-center rounded-full transition-colors focus:outline-none shadow-lg ${detailedTherapist.is_active ? 'bg-primary' : 'bg-white/20'}`}
                                        >
                                            <span className={`inline-block h-6 w-6 transform rounded-full bg-black transition-transform ${detailedTherapist.is_active ? 'translate-x-7' : 'translate-x-1'}`} />
                                        </button>
                                    </div>
                                </div>
                            </div>
