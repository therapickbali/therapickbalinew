            ...
            {service && (
                <>
                    ...
                    <motion.div
                        ...
                        exit={{ opacity: 0 }}
                        onClick={onClose}
                        className="fixed inset-0 bg-white/45 backdrop-blur-md z-[60]"
                    />
                    ...
                    <motion.div
                        ...
                        className="fixed inset-0 z-[100] bg-black overflow-y-auto custom-scrollbar font-sans"
                    >
                        {/* Close Button */}
                        <button
                            onClick={onClose}
                            className="fixed top-6 right-6 z-[110] p-2.5 bg-black/90 backdrop-blur-md rounded-full text-white/90-muted hover:text-white hover:bg-surface border border-white/20/80 transition-all shadow-soft"
                        ...
                        <div className="max-w-6xl mx-auto px-6 sm:px-8 lg:px-12 py-10 pb-32 lg:pb-16 lg:py-16">
                            ...
                            <div className="mb-8 pt-4 lg:pt-0">
                                <span className="uppercase tracking-[0.25em] font-sans font-bold text-[9px] text-accent bg-surface/80 border border-white/20/40 px-3.5 py-1.5 rounded-full inline-block mb-4">
                                    {service.category}
                                </span>
                                ...
                                <div className="flex items-center gap-2.5 text-xs font-medium text-white/90-muted">
                                    <div className="flex items-center gap-1">
                                        ...
                                        <span className="font-sans font-bold text-white">5.0</span>
                                    </div>
                            ...
                            <div className="flex flex-col lg:flex-row gap-10 lg:gap-16 relative">
                                ...
                                <div className="w-full lg:w-[60%]">
                                    <div className="pb-8 border-b border-white/20/60">
                                        <p className="font-sans text-white/90-muted text-base leading-relaxed font-light whitespace-pre-line first-letter:text-5xl first-letter:font-serif first-letter:text-white first-letter:float-left first-letter:mr-3.5 first-letter:mt-1">
                                            {mainDescription}
                                        </p>
                                    </div>
                                    ...
                                    {benefitsList.length > 0 && (
                                        <div className="py-10">
                                            <div className="flex items-center gap-4 mb-8">
                                                <h3 className="font-serif text-2xl font-normal text-white">Sacred Benefits</h3>
                                                <div className="h-[1px] bg-border/60 flex-1"></div>
                                            </div>

                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                                {benefitsList.map((benefit, i) => (
                                                    <div key={i} className="flex items-start gap-4 group p-3 bg-surface/35 border border-white/20/40 rounded-xl hover:border-accent/40 transition-colors duration-300">
                                                        <div className="shrink-0 mt-0.5">
                                                            {getBenefitIcon(benefit)}
                                                        </div>
                                                        <span className="font-sans text-white/90-muted font-normal text-xs leading-relaxed block">
                                                            {benefit}
                                                        </span>
                                                    </div>
                                                ))}
                                ...
                                <div className="w-full lg:w-[40%] pb-12 lg:pb-0">
                                    <div className="sticky top-8 bg-surface/40 border border-white/20/60 rounded-2xl p-6 lg:p-8 shadow-soft relative overflow-hidden">
                                        <div className="mb-8">
                                                ...
                                                {options.map((opt, i) => (
                                                    <button
                                                        ...
                                                        className={`w-full flex justify-between items-center px-5 py-4 rounded-xl text-xs font-bold uppercase tracking-wide transition-all duration-300 border ${
                                                            selectedOptionIdx === i
                                                                ? 'bg-black text-white border-accent/80 shadow-soft ring-1 ring-accent/10'
                                                                : 'bg-black/40 text-white/90-muted/65 border-white/20/50 hover:border-white/20 hover:bg-black'
                                                        }`}
                                                    >
                                                        <span className="flex items-center gap-2.5">
                                                            <div className={`w-3 h-3 rounded-full transition-all border ${
                                                                selectedOptionIdx === i 
                                                                    ...
                                                                    : 'bg-transparent border-white/20 group-hover:border-accent'
                                                            }`}></div>
                                                            {opt.duration}
                                                        </span>
                                        ...
                                        <div className="flex flex-col items-center gap-1.5 mb-6 pt-6 border-t border-white/20/60">
                                        ...
                                        <button
                                            onClick={handleAdd}
                                            className="w-full h-14 bg-white hover:bg-secondary text-white rounded-xl font-sans font-bold text-[10px] uppercase tracking-[0.22em] transition-all duration-300 flex items-center justify-center gap-2.5 shadow-soft"
                                        >
                                            <span>Add to Ritual</span>
                                            <ArrowRight size={14} strokeWidth={2} />
                                        </button>

                                        <p className="text-center text-[9px] font-medium tracking-[0.1em] text-white/90-muted/60 mt-4 uppercase">
                        ...
                        <div className="fixed bottom-0 left-0 right-0 bg-black border-t border-white/20/60 p-4 pb-6 lg:hidden z-[120] shadow-soft-lg">
                            <div className="max-w-md mx-auto">
                                    ...
                                    {options.map((opt, i) => (
                                        <button
                                            ...
                                            className={`whitespace-nowrap flex-1 py-2.5 px-4 rounded-full text-[9px] font-bold uppercase tracking-[0.18em] transition-all duration-300 border ${
                                                selectedOptionIdx === i
                                                    ? 'bg-white text-black border-primary shadow-soft'
                                                    : 'bg-black text-white/90-muted/65 border-white/20/80'
                                            }`}
                                        >
                                            {opt.duration}
                                        </button>
                                ...
                                <div className="flex items-center justify-between gap-4">
                                    ...
                                    <button
                                        onClick={handleAdd}
                                        className="flex-1 max-w-[150px] py-3.5 bg-white text-black rounded-xl font-sans font-bold text-[9px] uppercase tracking-[0.2em] hover:bg-secondary transition-all flex items-center justify-center shadow-soft"
                                    >
                                        Reserve Now
                                    </button>
                                </div>
